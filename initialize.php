...
$menus = [
    ...
    "Course"=>array(
        "Name" => "COURSES",
        "Path" => \_::$Router->CategoryRoot."Course",
        "Image" => "book",
        "Items" => []
    ),
    array(
...
foreach(array_slice(compute("category/all", ["Direction"=>"Course"]), 1) as $item)
    $courses[] = ["Name"=>$item["Title"], "Path"=>\_::$Router->CategoryRoot."Course/".$item["Name"]];
...
\_::$Info->Shortcuts = [
    ...
    array("Name" => "COURSES", "Path" => \_::$Router->CategoryRoot."Course", "Image" => "book"),
